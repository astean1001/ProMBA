...
let is_problem s = 
	try 
		ignore (BatString.find s "target"); true 
...
class problemVisitor = object(self)
	...
	method vfunc fd =
		...
		then
			let _ = problemSet := (BatMap.add fd.svar.vname (get_ret_expr fd.sallstmts) !problemSet) in
			DoChildren
		else DoChildren
end

let parse_one_file fname =
	let _ = debug "Parsing File: %s" fname in
...
let get_ret_expr stmts =
	...
	if List.length retlist < 1 then 
		let _ = debug "There is no return function in this problem!" in
		assert false
	else
		let found = List.hd retlist in
		match found with
		| Return (exp, _) -> (Option.get exp)
		| _ -> let _ = debug "Return value is empty!" in assert false
